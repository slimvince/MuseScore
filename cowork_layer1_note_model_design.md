...
- *Widen the covered span of music* — extend the analysed range **earlier in time and/or later in time** than the
  ...
  context than the user's selection provided.
...
- **The build currently reads the whole score even when only part of it is queried** — a known inefficiency, to be
  addressed as the selection-based working model matures.
