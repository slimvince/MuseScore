...
sub=np.load("/tmp/subf_fast.npy",allow_pickle=True); feat=np.load("/tmp/featf_fast.npy",allow_pickle=True)
...
print("5041 pieces  clusters<->tradition ARI=%.3f"%ARI(trad,lab))
for c in sorted(set(lab)):
    ...
    print(" c%d n=%d [%s]\n    idioms: %s"%(c,n,mk," ".join(feat[i] for i in cand[:7])))
# where do the new classical sub-corpora land (galant / baroque)?
print("\n=== where the Baroque/galant sub-corpora land ===")
for s in ["scarlatti","corelli","cpe_galant","bach_suites","mozart"]:
    d=collections.Counter(lab[sub==s]); tot=sum(d.values())
    if tot: print("  %-12s -> %s"%(s,"  ".join("c%d:%.0f%%"%(c,100*v/tot) for c,v in d.most_common(3))))
