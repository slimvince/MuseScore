...
*Last updated: 2026-07-13 (CC — **OI-155 CLOSED — abstain-on-truly-unknown RESTORED in the ONE shared key reduction** (`cc_oi155_report.md`). The user's ruling, implemented in `tools/` only (a grading-CONVENTION fix on the measurement chain; no `src/` change of any kind). `compare_rn._mode_is_major` — a two-valued PREFIX rule with no unknown state, so any unrecognized mode suffix fell through to MINOR — is replaced by the three-valued `_mode_class`: set-membership in **the producer's own emitted vocabulary**, read from `keymodeformatting.cpp` `keyModeSuffix()` (the 21 spellings) and partitioned by `keymodeanalyzer.h` `keyModeIsMajor()` (the producer's own major-third/minor-third split). A suffix in neither set and not one of the five OI-132 exotics now ABSTAINS on the mode axis (OI-33), and `compare_rn._our_key_ident` is the ONE abstain decision every graded site routes through (the OI-52 pattern applied to the key axis) — so an abstain is a `keyfail`, excluded from the key-agree denominator and flagged by `robust_stop_diff` if it rises, never completed into a confident reading. **NO GRADE MOVED, proven at the strongest resolution:** all 16 `a8_rebaseline_measure` output files are **byte-identical (sha256)** before vs after on all three presets — summary, every run/cell enumeration, every mapping — because all 14 mode suffixes the corpus actually emits are in the vocabulary, so the abstain path fires on no real cell. Battery PASS (a8_diff +0/−0 ×3, class-(b) Δ+0, calib 4/4, validate 3/3); root 66.04/64.98/65.93, RN 46.33/44.10/46.23, key-home 71.42/67.83/70.65, key-local 65.99/62.98/65.71 and key-abstain 0.0/0.0492/0.0289 % all unmoved; **no re-baseline, no snapshot owed.** The two red regression tests are green **by the code**, not by an edited expectation: `parse_our_key("Cweird")` is `(0, None)` again, and the genuinely stale one now expects the parent-collection answer `EPhrygDom` → `(9, False)` = A minor (OI-132). Python metric suites 119/119. **A new test parses BOTH C++ producer sources** and pins the grader's table complete-and-faithful to them, so a 22nd emitted mode goes red instead of grading silently — the mirror is mechanical, not a "keep in sync" comment. **Three findings declared:** (a) a THIRD hidden red assertion in the same test — the same OI-132 fold had also dropped the reduction's unicode-accidental normalization (`F♯maj` → `(None, None)`), masked behind the first failure; restored in the same one reduction, zero effect on real cells. (b) **OI-157 (new)** — a third copy of the mode classification, in `measure_joint_probe.py`'s enum-index table, went STALE at OI-132 (it still encodes the superseded same-tonic prefix rule and cannot express the parent-collection tonic move); read-only probe, no committed figure affected, declared not fixed. (c) a latent wrong-tonic path for double-sharp tonics (`Fx`), unreachable today, recorded on OI-152's territory. **Two rows filed per the user's defer ruling:** **OI-156** (the bridge's 4th hard-coded `0.25` onset literal, `src/notation`, gated on the next config-unification touch) and an **OI-34** amendment (the committed corpus is CRLF only because regeneration runs on Windows — the deferred obligation of the O-12 corpus git-tracking decision, cross-referenced from OI-137(a)). OI-152's dangling `_KB_MAJOR_MODE_PREFIXES` anchor corrected. Suites green; no golden refresh owed.)*
