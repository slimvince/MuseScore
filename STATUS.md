...
**Test baseline (as of Iter 95 Step 2 / `9fc27888d0`):**
...
- Pipeline snapshot tests: 11/11 passing (1 additional test skipped —
  `PipelineDivergenceCObservation.GenerateReport`, intentional opt-in) — Step 2
  refreshed 3 goldens: `bach_bwv806_prelude` (alt-only C# major / C# minor +0.20
  score deltas — w_seq firing on alternatives in dense Baroque cadence),
  `bach_bwv806_gigue` (tick 960 winner `DMaj9 → E7/D`, `IVM9 → V42` — classic
  V42→I in A major), `mozart_k280_1` (alt-only with inversion-stack reshuffle).
  Step 1 had previously refreshed 5 goldens; `mozart_k280_1` refreshed in both
  steps
- Chord mismatch report: 4 RealDiff (pinned baseline), 127 ConventionDiff (Jazz catalog)

**BIR baselines (Baroque preset, batch path, lenient-OR align_regions; re-confirmed 2026-05-17 post-Iter-95-Step-2):**
- Baroque BIR=true=44, BIR=false=27
- Jazz BIR=true=68, BIR=false=13
...
Cumulative since Iter 91: Baroque BIR=false 188 → 27 (−161, ~86% reduction); Jazz
BIR=true 103 → 68 (−35, ~34% reduction). Iter 92 contributed −142 Baroque BIR=false
...
plumbing so the live chord track and status bar receive the same signal.
