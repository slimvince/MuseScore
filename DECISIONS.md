...
| Decisions recorded outside the specification that owns them | 371 |
...
| — of which a decision about the process, correctly homed | 135 |
...
The second-to-last row is about the DECISIONS; the last is about the ENTRIES. **315 of 668** entries carry a recorded event at which the entry itself — this quote, this restatement, this status — was reviewed and ratified. The remaining 353 do not carry one in their own provenance; the register-level ratification events listed in the preamble are the place to look for those, and nothing is inferred from them into the per-entry field.

That last row is the one meant to fall. **668 of 668** decisions here can point at the research, the measurement, or the constraint that decided them; the rest cannot, and say so. Filling a gap means recording the defense where the decision lives — never writing one afterwards from memory.
...
- Coverage figures above regenerated at commit `d3cfb1a31b1cf257e4609efe73b7229ef34f511e`.
- Backbone data: `tools/audit/decisions/backbone_decisions.json` (sha256 `36d356768ea86190…`).
