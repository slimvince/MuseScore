# User rulings — 2026-08-16 (the preparation-opening return): the caller-check question ruled

> **STATUS: RULING RECORD, an interim carrier (D-230).** Taken by the user in conversation on
> 2026-08-16, at the return of the batch `cc_instruction_preparation_opening.md`, whose report and
> full close were read whole and verified at the objects by explicit hash before any decision was
> put (path sets, numstats, the register's byte-identity proven by hashing, both artifacts'
> populations and tallies re-counted at the artifacts, the committed guard artifact's own summary
> read at the object; the full-close reading proved by quotation, eighteenth-stop Ruling 15). Each
> decision is presented as a self-contained visible surface with alternatives, costs named to
> principles, and a recommendation, the question put in a separate turn after the surface is read.
> **The classification of each ruling — decision or exercise — is OWED and not made here**, and no
> decisions-register entry is written (the eighteenth stop's filtering ruling stands).
>
...
## 1. Ruling — the caller-check: an enumeration does NOT count as a caller (Alternative A)

The returned batch's caller-check reported every ruled retirement candidacy HELD-BY-CALLERS, with
the naming dominated by records that enumerate the tracked tree by construction (the measurement
is `tools/audit/retirement_caller_check.json`, its `who_does_the_naming` table; per D-431 no
count is restated beyond what this section reports about). The batch returned the question
undecided, with the per-caller measurement published. The surface put three alternatives; the
writing side's desk trial over the artifact's own per-member data — a naming-breadth lens used as
a TRIAL ONLY, declared authored and not proposed as a rule — showed discrimination returns when
broad enumerators are set aside, in both directions: some candidacies would pass whole, others
stay held by callers including governing documents and `src/` files, among them cross-flagged
holders (one candidate naming another) and at least one base-name collision. **The user's word:
"A." RULED — adopted whole:**

- **A naming from a tree-enumerating record does not hold a candidate.** The check is RE-RUN
  under a **derived caller-kind classification**: a generated artifact (or a surface rendered
  from one) whose generator enumerates the tracked tree is an ENUMERATOR — its namings are
  published as data and hold nothing; every other caller's naming stands. **Never a naming-breadth
  threshold** (the twice-declined shape) **and never a hand-typed exemption list.**
...
- **The re-run publishes each surviving holder BY KIND** — a tool reading the file by name, a
  mandatory-read or boot listing, a prose citation — **and the follow-up question (does a prose
  citation hold, given that archive-with-record keeps it resolvable?) is DEFERRED to that
  evidence.** It is not decided by this ruling.
- **Nothing is archived by this ruling.** The re-run's PASSES verdicts confer nothing until the
  pruning wave's own dispatch executes them; every ruled condition (mined-first;
  members-seen-by-the-user-first for the stray root files) stands untouched.
- Alternatives B (an enumeration counts — the wave permanently unexecutable) and C (a
  naming-breadth threshold — a hand-picked number over varying data) are DECLINED as rated on
  the surface.

## 2. Ruling — the decisions-register filter: the KEEP side is RATIFIED; the recovery pass precedes any discard ruling (Alternative A)

The batch's filter classified all 677 register entries: 411 DECIDING-ACT-NAMED (proposed keep,
each with its quoted evidence), 182 NO-DECIDING-ACT-FOUND, 84 EVIDENCE-AMBIGUOUS (a bare date).
The surface put one question — the keep side — with the non-keep residue routed to investigation
first under the standing investigate-by-default mandate. **Before ruling, the user put a
clarifying question that sharpened the operationalization**, in the user's words: *"Things like
'the slicer output covers the domain with no gaps and no overlaps' and 'the note model is the
single source of truth' look necessary/useful regardless of if we managed to formally document
its decision... There is probably I high risk that many things have been decided but not formally
documented as a 'decision' who/why/what/when. Are you saying that the two examples I mentioned
blindly will be thrown away?"* The writing side's answer, accepted: nothing is discarded blindly
or today; the filter decides register AUTHORITY, never content — the statements stand at their
homes and every statement of every outgoing specification is met by the disposition discipline
during derivation regardless of its register row's fate. **The user's ruling, verbatim: "Then
I'll go with your recommendation: A."** Adopted whole:

- **The 411 DECIDING-ACT-NAMED entries are RATIFIED as the live record's confirmed side.** The
  classification's keep verdicts carry load; a keep remains re-questionable at the rulings sort
  (keep is the reversible direction).
- **The register's INDEX stays under the filtering ruling for the 266 non-keep entries** — no
  entry moves, nothing is discarded, and an unconfirmed row is still not cited as authority; the
  classification artifact records which side every row is on.
- **The DECIDING-ACT RECOVERY PASS is ordered over ALL 266 non-keep entries** (whole population,
  D-671; an investigation, ordered under the investigate-by-default mandate rather than put as a
  question): for each entry, its own cited sources — open-items rows, records, sections — are
  followed through the record and searched for a user act naming the entry's subject; published
  per entry as ACT-FOUND (quoted, located) or NOTHING-FOUND. **The soft-discard ruling is put at
  that return, over evidence.**
- **The clause from the clarifying exchange binds the eventual discard ruling:** every
  soft-discard record states, per entry, that it is a PROVENANCE verdict and not a judgment on
  soundness or usefulness — the statement stands at its home and is met by the derivation.
