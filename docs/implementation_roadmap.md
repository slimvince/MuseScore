...
4. **Voice-leading layer (axis 2)** — ✅ **the fuller voice-leading-idiom discovery is DONE + RATIFIED (2026-07-03,
   `records/cc/reports/cc_vl_idiom_discovery_report.md`; folded as findings v2.0):** VL organizes by **texture** (contrapuntal vs
   ...
   (2026-07-03, `records/cc/reports/cc_vl_foundation_build_report.md`):** VL-A voice-linear view + VL-B motion/interval profiles + the
...
- **Before the CC implementation↔spec gap-analysis (the next CC task):** **A-1** — the **cross-layer confidence &
  ...
  **✅ READY: `records/cc/instructions/cc_instruction_gap_analysis_spec_vs_impl.md`** (read-only; per-layer gap tables + the five review
  ...
  **★ E0 instruction WRITTEN + READY: `records/cc/instructions/cc_instruction_e0_fullspine_measure.md`** (Task A: the default-OFF
...
- **★ THE SCORE/CORPUS CENSUS (2026-07-02): `cowork_score_census.md`** — the once-and-for-all, enumerated-to-closure
  ...
  **★ Wave 1 ✅ EXECUTED + RATIFIED (2026-07-02, `records/cc/reports/cc_corpus_wave1_report.md`):** the DLC container CLOSED (40/40
  ...
  **★ Wave 2 ✅ EXECUTED + RATIFIED (2026-07-03, `records/cc/reports/cc_corpus_wave2_report.md` — user-scoped to the core set):** the
...
- **Metric:** **A-8** — move the gate to the **granularity-robust union-of-boundaries unit** (already built, L0–L1
  ...
  **★ MEASURED + RATIFIED DUAL-TRACK (user, 2026-07-03; `records/cc/reports/cc_a8_rebaseline_measure_report.md`, instrument
...
`records/cc/reports/cc_precision_headroom_dossier.md` measured the REAL (DCML-only, Default config, unfiltered)
...
| 3.4 | Retire gates A–L one at a time: a gate is removed only when the decoder reproduces its pinned fixes (Stage 1.1 tests are the proof obligations). Gates J and R expected to survive longest (structural, healthy). **Stage-1a obligations:** the post-bonus quality-guard winner scan is first-wins on exact ties (ordering sensitivity), and the diff-root append never appends sub-threshold candidates — the decoder must either reproduce or consciously re-decide both (`records/cc/reports/cc_stage1a_report.md` F2/F5) | Part 1; audit F7 | per-gate differential report |
