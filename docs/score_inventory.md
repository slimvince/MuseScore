...
| Measure the BIR gate | `tools/corpus/baroque/` or `…/jazz/` | per-preset `.ours.json` + `.music21.json` + manifest | `characterise_bir_false.py --corpus-dir …` (Baroque 13, Jazz 7) |
...
- **`baroque/`, `jazz/` — per-preset measurement dirs:** each holds that preset's
  ...
  manifest and refuses an incomplete/contaminated dir (Baroque 13, Jazz 7).
