...
// ═══════════════════════════════════════════════════════════════════════════════════════════════
// OI-199 Task 1 — the JOINT-DECODER FIRE-COUNT characterization, to the L4 standard, on BOTH corpora.
// Governing question (cc_instruction_oi199_joint_p3_p4.md): IS OI-215 ALONE? Per-filter admission
// rejections (candidateStates gates 1/2/3) + per-branch DP fires, under the DEFAULT-OFF fire counter
// (jointFireSetEnabled; reverted at close per OI-110). The per-event coverability decomposition
// (scanCoverage) separates the OI-215 member-overlap failure from any sibling (root-present / NCT-
// budget). MEASUREMENT ONLY — the counter is byte-identical off; the decode RESULT is untouched on.
//
// Regenerate (fit corpus + large scores):
//   ./pipeline_snapshot_tests.exe --gtest_also_run_disabled_tests --gtest_filter='*JointFireFit*'
//   JOINT_FIRE_MAX_EVENTS=2000 ./pipeline_snapshot_tests.exe --gtest_also_run_disabled_tests \
//       --gtest_filter='*JointFireLarge*'
namespace {
using mu::composing::analysis::joint::JointFireCounters;
using mu::composing::analysis::joint::CoverageScan;

QJsonObject fireToJson(const JointFireCounters& fc)
{
    QJsonObject o;
    o["candWindows"] = static_cast<double>(fc.candWindows);
    o["candOffered"] = static_cast<double>(fc.candOffered);
    o["rejRootPresent"] = static_cast<double>(fc.rejRootPresent);
    o["rejMemInvalid"] = static_cast<double>(fc.rejMemInvalid);
    o["rejMemberOverlap"] = static_cast<double>(fc.rejMemberOverlap);
    o["rejFit"] = static_cast<double>(fc.rejFit);
    o["admitted"] = static_cast<double>(fc.admitted);
    QJsonArray od;
    for (int k = 0; k < 13; ++k) { od.append(static_cast<double>(fc.onsetDiv[k])); }
    o["onsetDiversityHistogram"] = od;
    o["dpPieces"] = static_cast<double>(fc.dpPieces);
    o["dpComplete"] = static_cast<double>(fc.dpComplete);
    o["dpEmpty"] = static_cast<double>(fc.dpEmpty);
    o["dpSegments"] = static_cast<double>(fc.dpSegments);
    o["trInitial"] = static_cast<double>(fc.trInitial);
    o["trSameKey"] = static_cast<double>(fc.trSameKey);
    o["trKeyChange"] = static_cast<double>(fc.trKeyChange);
    o["trContentNegInf"] = static_cast<double>(fc.trContentNegInf);
    o["trNoBack"] = static_cast<double>(fc.trNoBack);
    o["stInsert"] = static_cast<double>(fc.stInsert);
    o["stImprove"] = static_cast<double>(fc.stImprove);
    return o;
}

QJsonObject coverageToJson(const CoverageScan& cs)
{
    QJsonObject o;
    o["nEvents"] = cs.nEvents;
    o["uncoverable"] = cs.uncoverable;
    o["uncovMemberOverlapPure"] = cs.uncovMemberOverlapPure;
    o["uncovMemberOverlapRich"] = cs.uncovMemberOverlapRich;
    o["uncovFitBlocked"] = cs.uncovFitBlocked;
    o["uncovRootOnly"] = cs.uncovRootOnly;
    o["complete"] = cs.complete;
    return o;
}

// list the DP/admission branches that fired ZERO times (the dead-branch set for a population).
QJsonArray deadBranches(const JointFireCounters& fc)
{
    QJsonArray dead;
    auto chk = [&](const char* name, long long v) { if (v == 0) { dead.append(QString::fromUtf8(name)); } };
    chk("rejRootPresent", fc.rejRootPresent);
    chk("rejMemInvalid", fc.rejMemInvalid);
    chk("rejMemberOverlap", fc.rejMemberOverlap);
    chk("rejFit", fc.rejFit);
    chk("dpEmpty", fc.dpEmpty);
    chk("dpComplete", fc.dpComplete);
    chk("trInitial", fc.trInitial);
    chk("trSameKey", fc.trSameKey);
    chk("trKeyChange", fc.trKeyChange);
    chk("trContentNegInf", fc.trContentNegInf);
    chk("trNoBack", fc.trNoBack);
    chk("stInsert", fc.stInsert);
    chk("stImprove", fc.stImprove);
    return dead;
}
} // namespace

// (A) THE FIT CORPUS — the 326 covered chorales (note_events.json), the population every prior
// characterization used. scanCoverage over ALL pieces (admission + coverage, cheap); full decode
// over the decodable set (env JOINT_FIRE_FIT_MAX_PIECES; 0 = all) for the DP-branch fires.
TEST(JointFireFit, DISABLED_Sweep)
{
    using namespace mu::composing::analysis::joint;
    const std::string corpusPath = std::string(PIPELINE_SNAPSHOT_CORPUS_ROOT)
                                   + "/tools/joint_estimator/note_events/note_events.json";
    LoadedCorpus corpus = loadPiecesFromNoteEvents(corpusPath);
    ASSERT_TRUE(corpus.ok) << "loadPiecesFromNoteEvents failed: " << corpus.error;

    long maxPieces = 0;   // 0 = decode all
    if (const char* env = std::getenv("JOINT_FIRE_FIT_MAX_PIECES")) { maxPieces = std::atol(env); }
    ...
    // ── admission + coverage over ALL pieces (scanCoverage; cheap) ──
    jointFireSetEnabled(true);
    jointFireReset();
    int piecesComplete = 0, totalUncoverable = 0;
    int aggPure = 0, aggRich = 0, aggFit = 0, aggRoot = 0, aggEvents = 0;
    QJsonArray perPiece;
    std::vector<std::string> stems;
    stems.reserve(corpus.pieces.size());
    for (const auto& kv : corpus.pieces) { stems.push_back(kv.first); }
    for (const std::string& stem : stems) {
        const Piece& piece = corpus.pieces.at(stem);
        ChordCache cache;
        std::optional<int> sig;   // note_events has no per-piece signature here; decode uses none/declared below
        const CoverageScan cs = scanCoverage(piece, vocab, cache, sig, /*segCap=*/4);
        if (cs.complete) { ++piecesComplete; }
        totalUncoverable += cs.uncoverable;
        aggPure += cs.uncovMemberOverlapPure; aggRich += cs.uncovMemberOverlapRich;
        aggFit += cs.uncovFitBlocked; aggRoot += cs.uncovRootOnly; aggEvents += cs.nEvents;
        if (cs.uncoverable > 0) {   // enumerate only the (expected zero) uncoverable pieces on (A)
            QJsonObject po; po["stem"] = QString::fromStdString(stem);
            po["coverage"] = coverageToJson(cs);
            perPiece.append(po);
        }
    }
    const JointFireCounters admission = jointFireSnapshot();

    // ── full decode over the decodable set (DP-branch fires) ──
    jointFireReset();
    int decoded = 0, decodeEmpty = 0;
    long processed = 0;
    for (const std::string& stem : stems) {
        if (maxPieces > 0 && processed >= maxPieces) { break; }
        ++processed;
        const Piece& piece = corpus.pieces.at(stem);
        ChordCache cache;
        const DecodeResult r = decodePiece(piece, adapter, vocab, cache, 4, std::nullopt, "");
        ++decoded;
        if (!r.complete) { ++decodeEmpty; }
    }
    const JointFireCounters decodeFc = jointFireSnapshot();
    jointFireSetEnabled(false);

    QJsonObject root;
    root["instrument"] = QStringLiteral("pipeline_snapshot_tests.cpp JointFireFit.DISABLED_Sweep");
    root["dispatch"] = QStringLiteral("cc_instruction_oi199_joint_p3_p4.md Task 1 population (A)");
    root["corpus"] = QStringLiteral("tools/joint_estimator/note_events/note_events.json (326 covered chorales)");
    root["seg_cap"] = 4;
    root["nPieces"] = static_cast<int>(corpus.pieces.size());
    root["decodedPieces"] = decoded;
    root["decodeEmptyPieces"] = decodeEmpty;
    QJsonObject cov;
    cov["nEvents"] = aggEvents;
    cov["piecesComplete"] = piecesComplete;
    cov["piecesUncoverable"] = static_cast<int>(corpus.pieces.size()) - piecesComplete;
    cov["totalUncoverableEvents"] = totalUncoverable;
    cov["uncovMemberOverlapPure"] = aggPure;
    cov["uncovMemberOverlapRich"] = aggRich;
    cov["uncovFitBlocked"] = aggFit;
    cov["uncovRootOnly"] = aggRoot;
    root["coverageAggregate"] = cov;
    root["admissionScan"] = fireToJson(admission);       // all-window admission over all 326 pieces
    root["decode"] = fireToJson(decodeFc);               // DP-branch fires over the decoded set
    root["decodeDeadBranches"] = deadBranches(decodeFc);
    root["uncoverablePieces"] = perPiece;

    const QString outPath = QStringLiteral(PIPELINE_SNAPSHOT_CORPUS_ROOT)
                            + QStringLiteral("/tools/audit/oi199/joint_firecount_fit.json");
    QDir().mkpath(QFileInfo(outPath).absolutePath());
    const QByteArray bytes = serializeSnapshot(root).toUtf8();
    QFile out(outPath);
    ASSERT_TRUE(out.open(QIODevice::WriteOnly | QIODevice::Truncate)) << out.errorString().toStdString();
    ASSERT_EQ(out.write(bytes), static_cast<qint64>(bytes.size()));
    out.close();
    std::cout << "[fit] pieces=" << corpus.pieces.size() << " complete=" << piecesComplete
              << " uncoverableEvents=" << totalUncoverable << " decoded=" << decoded
              << " decodeEmpty=" << decodeEmpty << "\nwrote " << outPath.toStdString() << std::endl;
}

// (B) THE 23 COMMITTED LARGE SCORES (tools/extra scores/large/) — the repertoire the large-score
// requirement (OI-209) names, characterized on this module for the first time. Per score:
// buildAdapterFacts + scanCoverage (admission+coverage, ALL 23 — the cheap, informative part, since
// the 13 empty-decode scores reject at admission); + full decode with fires for the decodable subset
// (env JOINT_FIRE_MAX_EVENTS, default 2000 — the possibly-intractable largest are never force-run,
// feedback_never_stop_long_running).
TEST(JointFireLarge, DISABLED_Sweep)
{
    using namespace mu::composing::analysis::joint;
    auto analysisCfg = muse::modularity::globalIoc()->resolve<
        mu::composing::IComposingAnalysisConfiguration>("composing");
    ...
    long capEvents = 2000;
    if (const char* env = std::getenv("JOINT_FIRE_MAX_EVENTS")) { capEvents = std::atol(env); }
    ...
    QJsonArray scoresArr;
    // aggregate admission over ALL 23 (scanCoverage) and DP over the decoded subset.
    JointFireCounters aggScan, aggDecode;
    auto addFc = [](JointFireCounters& a, const JointFireCounters& b) {
        a.candWindows += b.candWindows; a.candOffered += b.candOffered;
        a.rejRootPresent += b.rejRootPresent; a.rejMemInvalid += b.rejMemInvalid;
        a.rejMemberOverlap += b.rejMemberOverlap; a.rejFit += b.rejFit; a.admitted += b.admitted;
        for (int k = 0; k < 13; ++k) { a.onsetDiv[k] += b.onsetDiv[k]; }
        a.dpPieces += b.dpPieces; a.dpComplete += b.dpComplete; a.dpEmpty += b.dpEmpty;
        a.dpSegments += b.dpSegments; a.trInitial += b.trInitial; a.trSameKey += b.trSameKey;
        a.trKeyChange += b.trKeyChange; a.trContentNegInf += b.trContentNegInf;
        a.trNoBack += b.trNoBack; a.stInsert += b.stInsert; a.stImprove += b.stImprove;
    };

    for (const LargeScore& ls : kLargeCorpus) {
        const QString absPath = QStringLiteral(PIPELINE_SNAPSHOT_CORPUS_ROOT)
                                + QLatin1Char('/') + QString::fromUtf8(ls.relPath);
        QJsonObject o;
        o["id"] = QString::fromUtf8(ls.id);
        if (!QFileInfo::exists(absPath)) { o["error"] = QStringLiteral("score file missing"); scoresArr.append(o); continue; }
        MasterScore* score = ScoreRW::readScore(muse::String::fromQString(absPath), /*isAbsolutePath=*/true);
        if (!score) { o["error"] = QStringLiteral("load failed"); scoresArr.append(o); std::cout << "[jfl] LOAD-FAIL " << ls.id << "\n"; continue; }
        AdapterFacts fx = buildAdapterFacts(score, std::string(ls.id), /*excludeStaves=*/{});
        o["adapterOk"] = fx.ok;
        if (!fx.ok) { o["adapterError"] = QString::fromStdString(fx.error); scoresArr.append(o); delete score; continue; }
        const int nEvents = static_cast<int>(fx.piece.events.size());
        o["adapterEvents"] = nEvents;
        o["adapterNotes"] = static_cast<int>(fx.piece.notes.size());
        o["staves"] = static_cast<int>(score->nstaves());

        // scanCoverage (admission + coverage) — ALL 23, counters on, per-score reset.
        jointFireSetEnabled(true);
        jointFireReset();
        {
            ChordCache cache;
            const CoverageScan cs = scanCoverage(fx.piece, vocab, cache, fx.sigFifths, /*segCap=*/4);
            o["coverage"] = coverageToJson(cs);
        }
        const JointFireCounters scanFc = jointFireSnapshot();
        o["admissionScan"] = fireToJson(scanFc);
        addFc(aggScan, scanFc);

        // full decode with fires — only the decodable subset (never force-run the intractable largest).
        if (capEvents <= 0 || nEvents <= capEvents) {
            jointFireReset();
            ChordCache cache;
            const DecodeResult r = decodePiece(fx.piece, adapter, vocab, cache, 4, fx.sigFifths, fx.declaredMode);
            const JointFireCounters decFc = jointFireSnapshot();
            o["decoded"] = true;
            o["nSegments"] = static_cast<int>(r.segments.size());
            o["decodeComplete"] = r.complete;
            o["decode"] = fireToJson(decFc);
            addFc(aggDecode, decFc);
            std::cout << "[jfl] " << ls.id << " events=" << nEvents << " segs=" << r.segments.size()
                      << " complete=" << r.complete << "\n";
        } else {
            o["decoded"] = false;
            o["decodeSkipped"] = QStringLiteral("adapterEvents exceeds JOINT_FIRE_MAX_EVENTS (coverage-scan only)");
            std::cout << "[jfl] " << ls.id << " events=" << nEvents << " SCAN-ONLY (cap=" << capEvents << ")\n";
        }
        jointFireSetEnabled(false);
        scoresArr.append(o);
        delete score;
    ...
    QJsonObject root;
    root["instrument"] = QStringLiteral("pipeline_snapshot_tests.cpp JointFireLarge.DISABLED_Sweep");
    root["dispatch"] = QStringLiteral("cc_instruction_oi199_joint_p3_p4.md Task 1 population (B)");
    root["corpus"] = QStringLiteral("tools/extra scores/large/ (23 committed large scores)");
    root["seg_cap"] = 4;
    root["cap_events"] = static_cast<double>(capEvents);
    root["scores"] = scoresArr;
    root["admissionScanAggregate"] = fireToJson(aggScan);       // all-window admission over all 23
    root["decodeAggregate"] = fireToJson(aggDecode);            // DP-branch fires over the decoded subset
    root["decodeDeadBranches"] = deadBranches(aggDecode);
    const QString outPath = QStringLiteral(PIPELINE_SNAPSHOT_CORPUS_ROOT)
                            + QStringLiteral("/tools/audit/oi199/joint_firecount_large.json");
    QDir().mkpath(QFileInfo(outPath).absolutePath());
    const QByteArray bytes = serializeSnapshot(root).toUtf8();
    QFile out(outPath);
    ASSERT_TRUE(out.open(QIODevice::WriteOnly | QIODevice::Truncate)) << out.errorString().toStdString();
