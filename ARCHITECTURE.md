...
### §5.14 Pass 2b: Bass-Movement Sub-Boundary Detection

After the primary harmonic-rhythm pass assigns chord regions, a second scan
(`detectBassMovementSubBoundaries()` in `notationcomposingbridgehelpers.h/.cpp`)
splits existing regions wherever the bass pitch class changes mid-region.

**Trigger condition:** Any bass PC change fires a sub-boundary. There is no
minimum interval threshold — stepwise motion (e.g., C→D) triggers just as
readily as a leap (e.g., C→G). The only guard is `minGapTicks` (default 2
quarter notes), which prevents beat-by-beat fragmentation in fast passages.

**Passing-tone suppression:** Chromatic passing tones that survive the gap
filter are suppressed downstream at the chord-analysis level via
`bassPassingToneMinWeightFraction`. This keeps the detection layer simple
(any PC change = boundary candidate) while letting the weight-based filter
discard low-weight transient bass notes.

**Implementation:** `detectBassMovementSubBoundaries()` scans onset-only notes
(ties excluded) within each existing region. When the bass PC of a new note
differs from the region's opening bass PC and the gap since the last accepted
boundary exceeds `minGapTicks`, a new region boundary is inserted.

**Status:** Implemented in Session 22 (commit `9789ee1c3f` on master,
`64cd153868` compilation fix on submission-phase1).

### §5.15 Enharmonic Root Spelling — TPC-Guided Disambiguation
...
### §5.16 Sus4 Structural Penalty — Requiring the Defining Perfect Fourth
