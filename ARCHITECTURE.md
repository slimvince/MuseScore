...
### 2.12 Cross-Platform by Default
...
**No chord inferred under Ped. markings**

In Chopin and likely other Romantic piano scores, harmonic regions under explicit
sustain pedal markings (`Ped.` / `*` notation, Pedal spanner) show no inferred chord
in the chord track. The pedal spanner boundary ticks may be interfering with region
creation or causing valid regions to be absorbed. This is related to but distinct from
the beat-1 Jaccard over-segmentation problem. Diagnose before implementing the
pedal-aware Jaccard fix.
