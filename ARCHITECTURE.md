...
- Tonicization labels (V/V, V/ii, V/IV etc.) — all presets
- Cadence markers (PAC, IAC, HC, DC, PC) — uses existing chord staff
  cadence detection logic, exposed via annotate path
- Pivot chord labels — uses existing chord staff pivot detection logic,
  exposed via annotate path
- Augmented sixth chord labels (It+6, Fr+6, Ger+6) — Standard and Baroque
  presets only; Jazz and Nashville presets suppress and emit chromatic
  Roman numeral instead

Nashville annotation mode emits chord symbols only. No Roman numerals,
no cadence markers, no pivot labels, no augmented sixth labels.

Cadence detection uses a read-only lookahead of one harmonic region past
the selection boundary. Pivot detection uses the existing key inference
context window which already looks behind the selection start. Neither
requires significant extension of the analysis window.
