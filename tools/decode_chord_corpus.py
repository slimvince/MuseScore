#!/usr/bin/env python3
"""Layer-4 per-slice CHORD DECODER corpus driver (DIAGNOSTIC, read-only).

Runs the isolated Layer-4 per-slice chord decoder (`batch_analyze --decode-chords
--preset <P>`) over the canonical 353-stem music21 Bach-chorale corpus
(`tools/corpus/*.xml`) and writes the decoder's per-slice chord to
`<out>/<preset>/<stem>.decode.json` — a region shape the chord-root graders
(compare_analyses / oracle_root_metric) and the new NCT-membership metric read.

`--decode-chords` returns BEFORE any analysis runs (the analysis pipeline is never
invoked), so this is read-only: production analysis output is unchanged and the
decoder is NOT wired into the live analyzer. This driver only iterates the corpus,
invokes the diagnostic, and collects its per-stem JSON. Mirrors
decode_keymode_corpus.py (the Layer-3 driver).

On Windows, batch_analyze MUST be launched via Git Bash (a direct Python subprocess
of the Qt headless exe triggers an access violation) — mirrors run_bach_preset.py /
decode_keymode_corpus.py.
...
def _run_one(exe, xml_path, preset, extra_args=""):
    """Run batch_analyze --decode-chords on one stem; return (rc, parsed_json_or_None, err).

    extra_args (Layer-4 sweep): an optional string of decode-only
    ChordSliceDecoderPreferences overrides (e.g. "--chord-context-slices 0"). Read only
    on the --decode-chords path, so production stays byte-identical."""
    try:
        if platform.system() == 'Windows':
            bash = _find_git_bash()
            if bash:
                cmd = (f'{_to_unix_path(exe)} "{_to_unix_path(xml_path)}" '
                       f'--decode-chords --preset {preset} {extra_args}'.rstrip())
                r = subprocess.run([str(bash), '-c', cmd],
                                   stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                                   timeout=300)
            else:
                r = subprocess.run([str(exe), str(xml_path), '--decode-chords',
...
def main():
    ...
    ap.add_argument("--out", default=str(_REPO_ROOT / "tools" / "corpus_decode_chord"),
                    help="output root; per-stem JSON goes to <out>/<preset>/<stem>.decode.json")
    ap.add_argument("--jobs", type=int, default=8)
    ap.add_argument("--extra-args", default="",
                    help="Layer-4 sweep: extra decode-only flags passed verbatim to "
                         "batch_analyze (e.g. '--chord-context-slices 0'). Decode-path only.")
    args = ap.parse_args()
