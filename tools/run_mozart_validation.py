#!/usr/bin/env python3
"""
run_mozart_validation.py — Two-way comparison of our harmonic analysis against
the DCMLab Mozart Piano Sonatas corpus (DCML annotations).

Steps:
  1. For each .mscx file in tools/dcml/mozart_piano_sonatas/MS3/, run batch_analyze
     to get our analysis as JSON (written to tools/reports/mozart/).
  2. Parse the corresponding .harmonies.tsv from tools/dcml/mozart_piano_sonatas/harmonies/.
...
Note:
    `K533-3.mscx` is still treated as a GUI-problematic/corrupt score on Windows,
    but headless `batch_analyze` handles the native MSCX path successfully. Do not
    exclude it from this corpus validation unless the headless path regresses.

Usage:
    python tools/run_mozart_validation.py [OPTIONS]
...
_MOZART_MSCX   = _REPO_ROOT / "tools" / "dcml" / "mozart_piano_sonatas" / "MS3"
_MOZART_TSV    = _REPO_ROOT / "tools" / "dcml" / "mozart_piano_sonatas" / "harmonies"


# ══════════════════════════════════════════════════════════════════════════
# Helpers (identical pattern to run_beethoven_validation.py)
# ══════════════════════════════════════════════════════════════════════════
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Compare our harmonic analysis against Mozart piano sonatas DCML annotations."
    )
    ...
    mozart_dir = output_root / f"mozart_{timestamp}"
    ...
    mscx_files = sorted(_MOZART_MSCX.glob("*.mscx"))
    total = len(mscx_files)
    if total == 0:
        print(f"ERROR: no .mscx files found in {_MOZART_MSCX}", file=sys.stderr)
        sys.exit(1)

    print(f"\nMozart piano sonatas corpus: {total} movements\n")
    ...
    for idx, mscx in enumerate(mscx_files, 1):
        stem      = mscx.stem
        ours_path = mozart_dir / f"{stem}.ours.json"
        tsv_path  = _MOZART_TSV / f"{stem}.harmonies.tsv"

        if not tsv_path.exists():
            print(f"  [{idx:2d}/{total}] {stem}  — no .harmonies.tsv, skipping")
            continue

        # Step 1 — batch_analyze
        if not args.skip_cpp or not ours_path.exists():
            if exe is None:
                print(f"  [{idx:2d}/{total}] {stem}  — SKIP (no executable)")
                continue
            ok = _run_batch_analyze(exe, mscx, ours_path)
            if not ok:
                print(f"  [{idx:2d}/{total}] {stem}  — batch_analyze failed, skipping")
                continue

        if not ours_path.exists():
            print(f"  [{idx:2d}/{total}] {stem}  — no output JSON, skipping")
            continue
        ...
        try:
            ...
            print(f"  [{idx:2d}/{total}] {stem}  — DCML parse error: {exc}")
            continue
        ...
        try:
            ...
            print(f"  [{idx:2d}/{total}] {stem}  — JSON load error: {exc}")
            continue
    ...
    # ── Aggregate summary ─────────────────────────────────────────────────
    print(f"\n{'='*70}")
    print(f"Mozart piano sonatas -- aggregate results")
    print(f"{'='*70}")
    ...
    # ── Write JSON summary ────────────────────────────────────────────────
    summary_path = output_root / "reports" / f"mozart_{timestamp}.json"
