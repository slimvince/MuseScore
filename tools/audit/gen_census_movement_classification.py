...
# ★ THE AFTER SIDE IS PINNED TOO, and for the reason the BEFORE side is.  This artifact states what
# ONE COMPLETED ACT moved, between two fixed states.  Read at the live tree, its later side drifts
# with every commit that touches the governing record — MEASURED, not anticipated: the batch's own
# close wrote four entries into `STATUS.md` and turned this check red at the very next commit, with
# nothing about the act having changed.  Worse than noise, it is wrong: a document whose naming a
# LATER edit removes would be reported as moved BY THE DISCARD, which this artifact would then be
# saying something false about.
AFTER_COMMIT = "26afbc0f75577b9183dce3b9d81431d827447821"
AFTER_COMMIT_IS = ("the commit that PERFORMED the ruled soft-discard — "
                   "`cc_instruction_preparation_fifth.md` Task 1")
...
def build() -> dict:
    ...
    after_names = census.cited_names(governing_text_at(AFTER_COMMIT))
    ...
    data = json.loads(git("show", f"{AFTER_COMMIT}:{BACKBONE}"))
    live = data["decisions"]
    ...
    return {
        ...
        "the_two_states_compared": {
            ...
            "what_the_before_commit_is": BEFORE_COMMIT_IS,
            "after": AFTER_COMMIT,
            "what_the_after_commit_is": AFTER_COMMIT_IS,
            "★_how_the_comparison_is_made":
                "the CITATION SCAN itself is re-run over the governing record at BOTH states, each "
                "read from the git objects, rather than the rendered surfaces being diffed. The "
                "scan and the governing-record definition are IMPORTED from the census that owns "
                "them (#6), so this classification and that census cannot disagree about what "
                "counts as a naming.",
            "★_why_BOTH_sides_are_pinned":
                "this artifact states what ONE COMPLETED ACT moved. A live later side would drift "
                "with every commit that touches the governing record — and would then report a "
                "naming a LATER edit removed as having been moved BY THE DISCARD, which is a "
                "false statement rather than noise.",
        },
