...
`cc_instruction_preparation_eleventh_amended.md` Task 3).  The surface said of itself that nothing on
it was ruled and nothing had been done, and it had been ruled — at the inventory sitting of
2026-08-15, class by class.  **THE TREATMENT IS IMPORTED FROM `tools/audit/gen_rulings_sort.py` (#6),
which already carries it**: the qualification is written at the GENERATOR and rendered into the
document in the same act, and the former banner text is preserved beside it (#12) with the former
rendering left standing in git.  It is written at the generator and not at the file because a hand
edit to a generated document is not durable by construction.
...
def render(inv: dict, data: dict) -> str:
    ...
    add("")
    add("> **STATUS: RULED, 2026-08-15. NOTHING IS EXECUTED BY IT.** This surface was a ruling "
        "surface awaiting the user; **the user ruled it at the inventory sitting of 2026-08-15, "
        "whose record is `cowork_rulings_2026_08_15_inventory_sitting.md`** — the authority for "
        "what was ruled and what was not, and for which of the acts it ordered have since been "
        "performed, none of which is restated here (#6). **The banner it replaces read \"STATUS: "
        "RULING SURFACE, awaiting the user. NOTHING HERE IS RULED, AND NOTHING HERE HAS BEEN "
        "DONE.\" — true when it was written, and made untrue by the sitting; the former rendering "
        "stands in git at the commits that carried it (#12).** Every role, mining verdict and "
        "retirement flag below is **AUTHORED by "
