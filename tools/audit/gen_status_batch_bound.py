...
#
# ★★★★★★★★★★★★★★ RE-AIMED AGAIN 2026-09-07 by
# `cc_instruction_claude_md_boot_membership_2026_09_07.md` Task 3 — the close — and ALL SIX authored
...
# own entries. The aiming it replaces is the prune-at-amendment batch's, which RAN. `BASE_COMMIT` is
# this batch's LAST TASK COMMIT pushed before the close began — Task 1, the named membership written
# into `CLAUDE.md` — per this tool's docstring; the then-previous batch is the prune-at-amendment
# batch, whose close entry names it and whose four entries below it say `Same dispatch`, so FIVE
# entries are expected to move. **THE DECLARED PREFIX ADJUSTMENT IS EXPECTED TO FIRE**, which is why
# this batch's own entries were written into `STATUS.md` BEFORE `--apply` ran. **The second writing's
...
# act.
#
# ★★★★★★★★★★★★★★★★ RE-AIMED AGAIN 2026-09-08 by
# `cc_instruction_defense_share_sizing_third_2026_09_08.md` Task 3 — the close — and ALL SIX
# authored inputs moved together, `PREVIOUS_AIMINGS` being appended to rather than replaced (#12).
# This is an ORDINARY move: the forward clause performed on this batch itself, in the same act that
# writes its own entries. The aiming it replaces is the read-size repair batch's, which RAN.
# `BASE_COMMIT` is this batch's LAST TASK COMMIT pushed before the close began — the Task 1 and
# Task 2 commit, which carries the reader's amendment and the new defense-share measurement tool —
# per this tool's docstring; the then-previous batch is the read-size repair batch, whose close
# entry names it and whose FOUR entries below it say `Same dispatch`, so FIVE entries are expected
# to move. *(This sentence read "three … so FOUR" when the re-aiming was written and was corrected
# at the object after the move ran and reported five: that batch closed with a Task 0 entry as well
# as Tasks 1 to 3. A comment stating a count the tool's own run contradicts is the defect #10
# forbids, and it is corrected here rather than left standing.)* **THE DECLARED PREFIX ADJUSTMENT
# IS EXPECTED TO FIRE**, which is why this batch's own
# entries were written into `STATUS.md` BEFORE `--apply` ran. **The second writing's two nameless
# 2026-09-02 entries remain in `STATUS.md` and no aiming of this tool can identify them**, as the
# row for the third writing below records; that is a declared state and not a STOP, and it is
# unchanged by this act. *(This batch made THREE commits and its own entries name the tasks rather
# than the commits, which is what every previous aiming's membership derivation reads; the entries
# themselves record which commit each task landed in.)*
#
# ★★★★★★★★★★★★★★★★★ RE-AIMED AGAIN by
# `cc_instruction_defense_share_authored_ends_2026_09_08.md` Task 3 — the close — and ALL SIX
# authored inputs moved together, `PREVIOUS_AIMINGS` being appended to rather than replaced (#12).
# This is an ORDINARY move: the forward clause performed on this batch itself, in the same act that
# writes its own entries. The aiming it replaces is the defense-share sizing batch's, which RAN.
# `BASE_COMMIT` is this batch's LAST TASK COMMIT pushed before the close began — the Task 1 and
# Task 2 commit, which carries the authored-end repair of the defense-share measurement and the
# re-bannering of the spent dispatch — per this tool's docstring; the then-previous batch is the
# defense-share sizing batch, whose close entry names it and whose entries below it say
# `Same dispatch`. **NO COUNT OF THE ENTRIES EXPECTED TO MOVE IS WRITTEN HERE**, and that silence is
# deliberate: the previous re-aiming asserted four where the run reported five and had to be
# corrected at the object afterwards, and a comment stating a count the tool's own run contradicts
# is the defect #10 forbids. The membership is DERIVED from the entries' own text at the base
# commit, and the run's own artifact is where the number belongs (D-431).
#
# ★ AND `ACT_DATE` DIFFERS FROM THE EXECUTING DISPATCH'S DATE. The dispatch is dated 2026-09-08 and
# this batch ran on 2026-09-09. The field states the day the ACT happened, which is exactly the case
# its own comment below provides for: a header carrying a dispatch's date would say something false
# about the record (#10) on any batch where the two differ, and this is such a batch. NO CLAIM IS
# MADE THAT IT IS THE FIRST such batch — the older rows below record no date of their own, so the
# question cannot be settled here.
#
# **THE DECLARED PREFIX ADJUSTMENT IS EXPECTED TO FIRE**, which is why this batch's own entries were
# written into `STATUS.md` BEFORE `--apply` ran. **The second writing's two nameless 2026-09-02
# entries remain in `STATUS.md` and no aiming of this tool can identify them**, as the row for the
# third writing below records; that is a declared state and not a STOP, and it is unchanged by this
# act.
#
...
# act.
#
# ★★★★★★★★★★★★★★★★★★★ RE-AIMED AGAIN 2026-09-19 by
# `cc_instruction_backup_second_commit_and_push_2026_09_19.md` Task 3 — the close — and ALL SIX
# authored inputs moved together, `PREVIOUS_AIMINGS` being appended to rather than replaced (#12).
# This is an ORDINARY move: the forward clause performed on this batch itself, in the same act that
# writes its own entries. The aiming it replaces is the passage-guards batch's MOVE 2, which RAN and
# is ALREADY the last row of `PREVIOUS_AIMINGS`, recorded there by that batch in the act that made
# it. It is therefore NOT appended a second time: this batch's own aiming is appended instead, on the
# convention every aiming since 2026-09-07 follows, so the list still holds every aiming exactly once.
# `BASE_COMMIT` is this batch's TASK COMMIT — the backup commit of record files by explicit path —
# as the executing dispatch orders; the then-previous batch is the passage-guards batch, whose close
# entry names its resume dispatch — the name the membership derivation reads — and below which no
# entry says `Same dispatch`. **NO COUNT OF THE ENTRIES EXPECTED TO MOVE IS WRITTEN HERE**, on the
# reason the authored-ends re-aiming states in its own words: the membership is DERIVED from the
# entries' own text at the base commit (D-431).
#
# **THE DECLARED PREFIX ADJUSTMENT IS EXPECTED TO FIRE**, the passage-guards batch's entry carrying
# the prefix at the base commit, which is why this batch's own entry was written into `STATUS.md`
# BEFORE `--apply` ran. `ACT_DATE` and the executing dispatch's date AGREE here, both being
# 2026-09-19. **The second writing's two nameless 2026-09-02 entries remain in `STATUS.md` and no
# aiming of this tool can identify them**, as the row for the third writing below records; that is a
# declared state and not a STOP, and it is unchanged by this act.
#
# ★★★★★★★★★★★★★★★★★★★★ RE-AIMED AGAIN 2026-09-20 by
# `cc_instruction_framework_dp_c_correction_second_2026_09_20.md` Task 4 — the close — and ALL SIX
# authored inputs moved together, `PREVIOUS_AIMINGS` being appended to rather than replaced (#12)
...
# act that writes its own entry. The aiming it replaces is the second backup's, which RAN and is
...
# the list still holds every aiming exactly once. `BASE_COMMIT` is this batch's LAST TASK COMMIT —
# Task 3, the `FRAMEWORK.md` correction with the regenerated boot-pack manifest — per this tool's
# docstring; the then-previous batch is the second backup, whose close entry names its dispatch and
# below which no entry says `Same dispatch`. **NO COUNT OF THE ENTRIES EXPECTED TO MOVE IS WRITTEN HERE**, on
# the reason the authored-ends re-aiming states in its own words: the membership is DERIVED from the
# entries' own text at the base commit (D-431).
#
# **THE DECLARED PREFIX ADJUSTMENT IS EXPECTED TO FIRE**, the second backup's entry carrying the
...
# ★ RE-AIMED 2026-09-21 by `cc_instruction_l2_withheld_documents_2026_09_21.md` Task 3, at its
# 5(b), and ALL FIVE authored inputs moved together, `PREVIOUS_AIMINGS` being appended to rather
# than replaced (#12). The aiming it replaces is the boot-pack frozen-manifest batch's, which is
# ALREADY the last row of `PREVIOUS_AIMINGS` — that batch recorded its own aiming in its own act —
# so it is not appended a second time, and this batch's aiming is appended instead. `BASE_COMMIT`
# is this batch's own base: the commit both `master` and `origin/master` stood at when this batch
# opened, read at the two ref FILES with the file tools rather than at a branch tip (D-253), and
# the commit the executing dispatch names on its own face.
#
# ★★ THE THEN-PREVIOUS BATCH IS AGAIN NAMED BY THE DISPATCH ITS ENTRY SAYS OF ITSELF, AND AGAIN
# THAT IS NOT THE LITERALLY PREVIOUS DISPATCH — the same shape the aiming above met, stated again
# rather than left to be inferred from it. `cc_instruction_boot_pack_frozen_manifest_close_2026_09_20.md`
# ran last and wrote no entry of its own: it AMENDED the boot-pack frozen-manifest batch's entry in
# place, and that entry still opens by naming that batch. **Both names occur in that one entry's
# line**, the close's ★★ sentence naming its own dispatch inside it, so either string selects the
# same single entry and the membership is unaffected either way; what the choice decides is what the
# archive header SAYS, and it says the batch whose entry it is (#10).
#
# **THE DECLARED PREFIX ADJUSTMENT IS EXPECTED TO FIRE**, that entry carrying the `Last updated: `
# prefix at the base commit, which is why this batch's own entry was written into `STATUS.md` BEFORE
...
# ordering both halves of the close — this batch's own entries and this move — inside its Task 4. It
# names Task 3 while `cc_instruction_defense_share_sizing_third_2026_09_08.md` is the executing act,
...
# that dispatch ordering this batch's own entries and BOTH of its two moves inside its own Task 3. It
# names Task 3 again while `cc_instruction_backup_second_commit_and_push_2026_09_19.md` is the
...
PREVIOUS_AIMINGS = [
    ...
    {"executing_act": "cc_instruction_claude_md_prune_at_amendment_2026_09_07.md, Task 4",
     ...
     "the_kind_of_move": "ordinary"},
    # ★ THIS BATCH'S OWN AIMING, 2026-09-07 — the boot-membership batch's close, an ORDINARY move on
    # the shape the two rows above set, and recorded here in the same act that makes it.
    ...
    {"executing_act": "cc_instruction_read_size_repair_and_rows_2026_09_07.md, Task 4",
     ...
     "the_kind_of_move": "ordinary"},
    # ★ THIS BATCH'S OWN AIMING, 2026-09-08 — the defense-share sizing batch's close, an ORDINARY
    # move on the shape the four rows above set, and recorded here in the same act that makes it.
    {"executing_act": "cc_instruction_defense_share_sizing_third_2026_09_08.md, Task 3",
     "base_commit": "38e8d81ab7e867e282117e9110fd8cf105a74833",
     "the_then_previous_batch": "cc_instruction_read_size_repair_and_rows_2026_09_07.md",
     "the_kind_of_move": "ordinary"},
    # ★ THIS BATCH'S OWN AIMING, 2026-09-09 — the authored-ends batch's close, an ORDINARY move on
    # the shape the five rows above set, and recorded here in the same act that makes it. Its act
    # date differs from its executing dispatch's date — the dispatch dated 2026-09-08, the batch
    # having run on 2026-09-09 — which is the case `ACT_DATE`'s own comment provides for.
    ...
    {"executing_act": "cc_instruction_passage_guards_historical_resume_2026_09_17.md, Task 3",
     ...
     "the_kind_of_move": "ordinary"},
    # ★ THIS BATCH'S OWN AIMING, 2026-09-19 — the second backup's close, an ORDINARY move on the
    ...
    {"executing_act": "cc_instruction_backup_second_commit_and_push_2026_09_19.md, Task 3",
     "base_commit": "3887c38c063cb8f29386757338908cf45ca5b5f1",
     "the_then_previous_batch": "cc_instruction_passage_guards_historical_resume_2026_09_17.md",
     "the_kind_of_move": "ordinary"},
    # ★ THIS BATCH'S OWN AIMING, 2026-09-20 — the DP-C correction's close, an ORDINARY move on the
    # shape the rows above set, and recorded here in the same act that makes it. The aiming it
    # replaces is the row immediately above, which that batch recorded in its own act, so it is not
    # appended a second time.
    ...
    {"executing_act": "cc_instruction_boot_pack_frozen_manifest_2026_09_20.md, §6 (the close)",
     ...
     "the_kind_of_move": "ordinary"},
    # ★ THIS BATCH'S OWN AIMING, 2026-09-21 — the close of the batch that derived and printed L2's
    # withheld documents, an ORDINARY move on the shape the rows above set, and recorded here in the
    # same act that makes it. The aiming it replaces is the row immediately above, which that batch
    # recorded in its own act, so it is not appended a second time. Its `the_then_previous_batch` is
    # again NOT the literally previous dispatch: the boot-pack frozen-manifest batch's CLOSE ran
    # between the two and wrote no entry of its own, amending that batch's entry in place instead.
    # The reasoning is at `BASE_COMMIT` above and is not restated here.
