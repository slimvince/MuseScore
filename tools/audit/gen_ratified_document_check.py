...
def render_surface(a: dict, guard: dict, recovered: dict) -> str:
    ...
    add("# The soft-discard's residue — what the guard withheld, and what the check over the 62 "
        "found")
    add()
    add("> **STATUS: RULING SURFACE, awaiting the user. NOTHING HERE IS RULED.** No entry on this")
    add("> surface has been discarded, retired, edited, moved or marked, and the two derivations")
    add("> it reports performed no decisions-register mutation of any kind. Generated by")
