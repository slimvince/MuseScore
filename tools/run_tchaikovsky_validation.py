#!/usr/bin/env python3
"""
run_tchaikovsky_validation.py — Two-way comparison of our harmonic analysis against
the DCMLab Tchaikovsky Seasons corpus (DCML annotations).

Usage:
    python tools/run_tchaikovsky_validation.py [OPTIONS]
...
_TCHAIKOVSKY_MSCX   = _REPO_ROOT / "tools" / "dcml" / "tchaikovsky_seasons" / "MS3"
_TCHAIKOVSKY_TSV    = _REPO_ROOT / "tools" / "dcml" / "tchaikovsky_seasons" / "harmonies"
...
def main():
    ...
    out_dir = output_root / f"tchaikovsky_{timestamp}"
    ...
    mscx_files = sorted(_TCHAIKOVSKY_MSCX.glob("*.mscx"))
    total = len(mscx_files)
    if total == 0:
        print(f"ERROR: no .mscx files in {_TCHAIKOVSKY_MSCX}", file=sys.stderr); sys.exit(1)

    print(f"\nTchaikovsky Seasons corpus: {total} movements\n")
    ...
    for idx, mscx in enumerate(mscx_files, 1):
        ...
        tsv_path  = _TCHAIKOVSKY_TSV / f"{stem}.harmonies.tsv"
    ...
    print(f"\n{'='*70}")
    print(f"Tchaikovsky Seasons -- aggregate results")
    print(f"{'='*70}")
    ...
    modal = _modal_distribution(out_dir)
    ...
    summary_path = output_root / "reports" / f"tchaikovsky_{timestamp}.json"
    summary_path.parent.mkdir(parents=True, exist_ok=True)
    summary = {
        'corpus': 'Tchaikovsky Seasons',
        'timestamp': timestamp,
        'git_hash': _get_git_hash(),
        'movements': len(all_results),
        'ours_dir': str(out_dir),
        'aggregate': {
