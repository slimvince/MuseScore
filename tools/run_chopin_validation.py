#!/usr/bin/env python3
"""
run_chopin_validation.py — Two-way comparison of our harmonic analysis against
the DCMLab Chopin Mazurkas corpus (DCML annotations).

Steps:
  1. For each .mscx file in tools/dcml/chopin_mazurkas/MS3/, run batch_analyze
     to get our analysis as JSON (written to a timestamped subdirectory).
  2. Parse the corresponding .harmonies.tsv from
     tools/dcml/chopin_mazurkas/harmonies/.
  ...
  4. Report per-movement and aggregate stats, including bassIsRoot breakdown
     and any notable modal findings (Lydian is the primary target for this
     corpus — calibrating modal priors before jazz work).

Usage:
    python tools/run_chopin_validation.py [OPTIONS]
...
_CHOPIN_MSCX     = _REPO_ROOT / "tools" / "dcml" / "chopin_mazurkas" / "MS3"
_CHOPIN_TSV      = _REPO_ROOT / "tools" / "dcml" / "chopin_mazurkas" / "harmonies"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Compare our harmonic analysis against Chopin mazurkas DCML annotations."
    )
    ...
    chopin_dir = output_root / f"chopin_{timestamp}"
    chopin_dir.mkdir(parents=True, exist_ok=True)
    ...
    mscx_files = sorted(_CHOPIN_MSCX.glob("*.mscx"))
    total = len(mscx_files)
    if total == 0:
        print(f"ERROR: no .mscx files found in {_CHOPIN_MSCX}", file=sys.stderr)
        sys.exit(1)

    print(f"\nChopin mazurkas corpus: {total} movements\n")
    ...
    for idx, mscx in enumerate(mscx_files, 1):
        stem      = mscx.stem
        ours_path = chopin_dir / f"{stem}.ours.json"
        tsv_path  = _CHOPIN_TSV / f"{stem}.harmonies.tsv"
    ...
    # ── Aggregate summary ─────────────────────────────────────────────────
    print(f"\n{'='*70}")
    print(f"Chopin mazurkas -- aggregate results")
    print(f"{'='*70}")
    ...
    # ── Write JSON summary ────────────────────────────────────────────────
    summary_path = output_root / "reports" / f"chopin_{timestamp}.json"
    summary_path.parent.mkdir(parents=True, exist_ok=True)
    summary = {
        'corpus': 'Chopin mazurkas',
        'timestamp': timestamp,
        'git_hash': _get_git_hash(),
        'movements': len(all_results),
        'ours_dir': str(chopin_dir),
        'aggregate': {
            'total': agg_total,
            'aligned': agg_aligned,
            'agree': agg_agree,
    ...
    print(f"ours JSON written to {chopin_dir}")
