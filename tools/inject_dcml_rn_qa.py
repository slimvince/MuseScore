...
# Hard-coded BWV → WiR folder mapping. Values may be a single folder string
# or a list of folders when multiple WiR analyses cover the same music21 score
# (e.g. bwv25.6 has both Riemenschneider 254 and 282 readings of the same tune).
WIR_MAP = {
    # Original five
    "bwv103.6": "120",
    "bwv283":   "198",
...
def main() -> None:
    ...
    for stem in stems:
        if stem not in WIR_MAP:
            ...
            fail += 1
            continue
        folders = WIR_MAP[stem]
        if isinstance(folders, str):
            folders = [folders]

        # Resolve our-analyzer JSON once per stem (shared across folder variants).
        ...
        for folder in folders:
            analysis_path = WIR_BASE / folder / "analysis.txt"
            if not analysis_path.exists():
                print(f"  missing WiR analysis: {analysis_path}", file=sys.stderr)
                fail += 1
                continue

            suffix = f"_r{folder}" if len(folders) > 1 else ""
            out_path = OUT_DIR / f"{stem}{suffix}_dcml.xml"
            print(f"Processing {stem} (folder {folder}) ...")
            try:
