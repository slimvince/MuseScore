#!/usr/bin/env python3
"""
run_bach_suites_validation.py — Two-way comparison against the DCMLab
Bach English and French Suites corpus (DCML annotations).

Usage:
    python tools/run_bach_suites_validation.py [OPTIONS]
...
_MSCX_DIR    = _REPO_ROOT / "tools" / "dcml" / "bach_en_fr_suites" / "MS3"
_TSV_DIR     = _REPO_ROOT / "tools" / "dcml" / "bach_en_fr_suites" / "harmonies"
...
def main():
    ...
    out_dir     = output_root / f"bach_suites_{timestamp}"
    ...
    print(f"\nBach En/Fr Suites corpus: {total} movements\n")
    ...
    print(f"\n{'='*70}\nBach En/Fr Suites -- aggregate results\n{'='*70}")
    ...
    sp = output_root/"reports"/f"bach_suites_{timestamp}.json"
    sp.parent.mkdir(parents=True, exist_ok=True)
    sp.write_text(json.dumps({
        'corpus':'Bach En/Fr Suites','timestamp':timestamp,'git_hash':_get_git_hash(),
