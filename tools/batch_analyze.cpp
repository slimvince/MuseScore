...
// ── Iter 51: replacement segmentation — data structures and candidate pool ───
...
static constexpr double kAnchorMinScore = 1.5;
...
static constexpr int kAnchorMinDurationTicks = 1 * Constants::DIVISION;
...
static constexpr double kRound2MinScore = 1.25;
                       ...
                       const std::set<size_t>& excludeStaves)
{
    ...
    for (const Segment* s = firstSeg; s && s->tick() < endTick;
        ...
        for (size_t staffIdx = 0; staffIdx < score->nstaves(); ++staffIdx) {
            if (excludeStaves.count(staffIdx) || !staffIsEligible(score, staffIdx)) {
                continue;
            }
            for (voice_idx_t v = 0; v < VOICES; ++v) {
                    ...
                    analysis::KeySigMode globalKeyMode)
{
    ...
    struct Scored {
        ...
        int           initialRootPc = -1;
        int           initialBassPc = -1;
        ChordQuality  initialQuality = ChordQuality::Unknown;
    };
    ...
    for (size_t i = 0; i < regions.size(); ++i) {
        ...
        auto tones = collectRegionTones(score, r.startTick, r.endTick, excludeStaves);
...
/// those satisfying beat/duration/voice/score criteria to round=1 anchors.
                         ...
                         analysis::KeySigMode globalKeyMode)
{
    ...
    const auto changeTicks = collectNoteChangeTicks(score, startTick, endTick, excludeStaves);
    ...
    for (PlacedRegion& region : candidates) {
        ...
        if (countParticipatingStaves(score, regionStart, regionEnd, excludeStaves) < 3) {
            continue;
        }

        auto tones = collectRegionTones(score, region.startTick, region.endTick, excludeStaves);
        if (tones.empty()) {
            continue;
        }
