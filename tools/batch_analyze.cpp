...
static void printHelp(const std::string& prog)
{
    std::cerr
        ...
        << "  --key-in-neither N | --key-in-keysig-only N | --key-in-candidate-only N |\n"
        << "  --key-in-both N | --key-leading-tone N\n"
        << "            (Layer-3 BOUNDED SWEEP) Override the KeyModeAnalyzer scale-membership\n"
        << "            emission weights (scaleScoreInNeither / InKeySigOnly / InCandidateOnly\n"
        << "            / InBoth, and leadingToneWeight) for the --decode-keymode path only:\n"
        << "            they are applied to a COPY of the key prefs inside that block, so the\n"
        << "            prefs production analysis uses are untouched and the corpus stays\n"
        << "            byte-identical. Measures the §3 A-intersect-stable emission reweight.\n"
        << "            Default = the preset value.\n"
        << "  --reachback-ab\n"
        << "            (Layer-3 A/B, read-only DIAGNOSTIC — returns before analyzeScore) Run\n"
        << "            the SHARED region analyzer over a set of interior single-measure range\n"
        << "            selections TWICE — reach-back OFF (production) and ON (the gated\n"
        << "            capability) — and report, per range, whether the emitted analysis\n"
        << "            differs, plus the wall-time of each. Never reaches the corpus path, so\n"
        << "            production is byte-identical. Activating reach-back on the live\n"
        << "            range-query path is a SEPARATE ratification; this only measures it.\n"
        << "            Default OFF.\n"
        << "  --decode-chords\n"
