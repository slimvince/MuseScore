#!/usr/bin/env python3
"""
run_dvorak_validation.py — Two-way comparison of our harmonic analysis against
the DCMLab Dvorak Silhouettes corpus (DCML annotations).

Usage:
    python tools/run_dvorak_validation.py [OPTIONS]
...
_DVORAK_MSCX    = _REPO_ROOT / "tools" / "dcml" / "dvorak_silhouettes" / "MS3"
_DVORAK_TSV     = _REPO_ROOT / "tools" / "dcml" / "dvorak_silhouettes" / "harmonies"
...
def main():
    ...
    out_dir = output_root / f"dvorak_{timestamp}"
    ...
    mscx_files = sorted(_DVORAK_MSCX.glob("*.mscx"))
    total = len(mscx_files)
    if total == 0:
        print(f"ERROR: no .mscx files in {_DVORAK_MSCX}", file=sys.stderr); sys.exit(1)

    print(f"\nDvorak Silhouettes corpus: {total} movements\n")
    ...
    for idx, mscx in enumerate(mscx_files, 1):
        ...
        tsv_path  = _DVORAK_TSV / f"{stem}.harmonies.tsv"
    ...
    print(f"\n{'='*70}")
    print(f"Dvorak Silhouettes -- aggregate results")
    print(f"{'='*70}")
    ...
    summary_path = output_root / "reports" / f"dvorak_{timestamp}.json"
    summary_path.parent.mkdir(parents=True, exist_ok=True)
    summary = {
        'corpus': 'Dvorak Silhouettes',
        'timestamp': timestamp,
